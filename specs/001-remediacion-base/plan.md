...
- **React 19 + `@vitejs/plugin-react`**: declarar React explícitamente (antes
  ...
  automático. *Alternativa descartada:* depender del `esbuild` de Vite sin
...
- **Tailwind v4 CSS-first**: `@tailwindcss/postcss` + `@import "tailwindcss";`.
  ...
  `autoprefixer` (integrado). *Alternativa descartada:* `@tailwindcss/vite`
  (válida, pero obliga a cambiar más piezas).
- **`tsc --noEmit` sobre `.jsx`**: se añade `allowJs: true`, `checkJs: false` y
  `jsx: "react-jsx"` para que el typecheck tenga entradas y no falle. *Decisión
  abierta:* migrar a TS real (ver dudas de la spec).
...
- **Cobertura de la API de OpenWeather**: el *free tier* limita UV/polen; puede
